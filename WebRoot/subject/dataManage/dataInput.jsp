...
$(function() {
	...
	var d = date.getDate();
	var today = m+'/'+d+'/'+y;
	var s_date = '${form.occurdate}';
	$('#dateBox').datebox({
		required:true,
		currentText:'Today'
	}).datebox('setValue',s_date==""?today:s_date);
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	var s2 = $("#sp input[value='${form.direction}']").next('span').text();
	$('#direction').combobox('setValue', '${form.direction}').combobox('setText', s2).combobox('hidePanel');
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
});
//-----------------提交表单---------------------------------
...
function submitForm(){
	dataEntryForm.action='${pageContext.request.contextPath }/servlet/InsertAccountDetail';
...
<!--  	<div class="easyui-panel" title="发生额数据录入" style="width:600px;padding:10px 60px 20px 60px">		-->
		<form id="dataEntryForm" name="dataEntryForm" method="post">
			...
			<table>
				...
				<tr>
					<td>所属单位：</td>
					<td><input id="comboTree" name="groupid" value='${form.groupid}' style="width:200px;"><span>${form.errors.groupid}</span></td>
				</tr>
				<tr>
					<td>摘要：</td>
					<td><input id="smy" name="summary" class="easyui-validatebox textbox" value='${form.summary}' data-options="required:true,validType:'length[1,30]'"><span>${form.errors.summary}</span></td>
				</tr>
				<tr>
					<td>方向：</td>
					<td>
				...
				<tr>
					...
					<td><input id="amount" name="amount" type="text" class="easyui-numberbox" value='${form.amount}' data-options="required:true,min:0,precision:4,groupSeparator:','"><span>${form.errors.amount}</span></td>
				</tr>
				<tr class="trCenter">
